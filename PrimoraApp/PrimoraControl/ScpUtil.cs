...
using Primora.PrimoraControl;
using Primora.InputDevices;
using Primora.StickModifiers;
using Primora.PrimoraControl.DTOXml;
using Primora.PrimoraForms.ViewModels;
using Sensorit.Base;
using System;
...
using Primora;
using Primora.ApiDTO;
using WpfScreenHelper;
using static Primora.Mouse;
using static Primora.Util;
using LightbarMacro = Primora.PrimoraForms.ViewModels.LightbarMacro;
...
namespace Primora
{
    ...
    public class PrimoraControlSettings
    {
        ...
        public PrimoraControlSettings(PrimoraControls ctrl)
        {
            control = ctrl;
        }
    ...
    {
        public List<PrimoraControlSettings> LS = new List<PrimoraControlSettings>();
        public List<PrimoraControlSettings> RS = new List<PrimoraControlSettings>();
        public PrimoraControlSettings L2;
        public PrimoraControlSettings L2FullPull;
        public PrimoraControlSettings R2;
        public PrimoraControlSettings R2FullPull;

        public PrimoraControlSettings GyroSwipeLeft;
        public PrimoraControlSettings GyroSwipeRight;
        public PrimoraControlSettings GyroSwipeUp;
        public PrimoraControlSettings GyroSwipeDown;

        public List<PrimoraControlSettings> ControlButtons =
            new List<PrimoraControlSettings>();

        public List<PrimoraControlSettings> ExtraDeviceButtons =
            new List<PrimoraControlSettings>();

        private List<PrimoraControlSettings> settingsList;

        public ControlSettingsGroup(List<PrimoraControlSettings> settingsList)
        {
            LS.Add(settingsList[(int)PrimoraControls.LSOuter - 1]);
            for (int i = (int)PrimoraControls.LXNeg; i <= (int)PrimoraControls.LYPos; i++)
            {
                LS.Add(settingsList[i - 1]);
            }

            RS.Add(settingsList[(int)PrimoraControls.RSOuter - 1]);
            for (int i = (int)PrimoraControls.RXNeg; i <= (int)PrimoraControls.RYPos; i++)
            {
                RS.Add(settingsList[i - 1]);
            }

            L2 = settingsList[(int)PrimoraControls.L2 - 1];
            R2 = settingsList[(int)PrimoraControls.R2 - 1];

            L2FullPull = settingsList[(int)PrimoraControls.L2FullPull - 1];
            R2FullPull = settingsList[(int)PrimoraControls.R2FullPull - 1];

            GyroSwipeLeft = settingsList[(int)PrimoraControls.GyroSwipeLeft - 1];
            GyroSwipeRight = settingsList[(int)PrimoraControls.GyroSwipeRight - 1];
            GyroSwipeUp = settingsList[(int)PrimoraControls.GyroSwipeUp - 1];
            GyroSwipeDown = settingsList[(int)PrimoraControls.GyroSwipeDown - 1];

            ControlButtons.Add(settingsList[(int)PrimoraControls.L1 - 1]);
            ControlButtons.Add(settingsList[(int)PrimoraControls.L3 - 1]);
            ControlButtons.Add(settingsList[(int)PrimoraControls.R1 - 1]);
            ControlButtons.Add(settingsList[(int)PrimoraControls.R3 - 1]);

            // Populate basic buttons used for mapping before DualSense Edge extra
            // buttons in PrimoraControls enum
            for (int i = (int)PrimoraControls.Square; i <= (int)PrimoraControls.Mute; i++)
            {
            ...
            // buttons in PrimoraControls enum
            for (int i = (int)PrimoraControls.GyroXPos; i <= (int)PrimoraControls.SwipeDown; i++)
            {
                ControlButtons.Add(settingsList[i - 1]);
            }
        ...
        public void EstablishExtraButtons(List<PrimoraControls> buttonList)
        {
            foreach (PrimoraControls control in buttonList)
            {
    ...
    public class Global
    {
        ...
        // Need to perform extra steps to check if Primora is installed in a junction
        ...
        public static string appDataPpath = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\Primora";
        public static string localAppDataPpath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Primora");
        ...
        public static OutContType[] outDevTypeTemp = new OutContType[TEST_PROFILE_ITEM_COUNT] { Primora.OutContType.X360, Primora.OutContType.X360,
            ...
            Primora.OutContType.X360, Primora.OutContType.X360,
            Primora.OutContType.X360, Primora.OutContType.X360,
            Primora.OutContType.X360};
        // Used to hold the currently active controller output type in use for a slot
        public static OutContType[] activeOutDevType = new OutContType[TEST_PROFILE_ITEM_COUNT] { Primora.OutContType.None, Primora.OutContType.None,
            Primora.OutContType.None, Primora.OutContType.None,
            Primora.OutContType.None, Primora.OutContType.None,
            Primora.OutContType.None, Primora.OutContType.None,
            Primora.OutContType.None};
        ...
        public const string ASSEMBLY_RESOURCE_PREFIX = "pack://application:,,,/Primora;";
        public const string RESOURCES_PREFIX = "/Primora;component/Resources";
        // Need to add additional probing path in code starting with .NET 6.
        public const string PROBING_PATH = "Lang";
        public const string LANGUAGE_ASSEMBLY_NAME = "Primora.resources.dll";
        public const string CUSTOM_EXE_CONFIG_FILENAME = "custom_exe_name.txt";
        public const string XML_EXTENSION = ".xml";

        public static X360Controls[] defaultButtonMapping = {
            X360Controls.None, // PrimoraControls.None
            X360Controls.LXNeg, // PrimoraControls.LXNeg
            X360Controls.LXPos, // PrimoraControls.LXPos
            X360Controls.LYNeg, // PrimoraControls.LYNeg
            X360Controls.LYPos, // PrimoraControls.LYPos
            X360Controls.RXNeg, // PrimoraControls.RXNeg
            X360Controls.RXPos, // PrimoraControls.RXPos
            X360Controls.RYNeg, // PrimoraControls.RYNeg
            X360Controls.RYPos, // PrimoraControls.RYPos
            X360Controls.LB, // PrimoraControls.L1
            X360Controls.LT, // PrimoraControls.L2
            X360Controls.LS, // PrimoraControls.L3
            X360Controls.RB, // PrimoraControls.R1
            X360Controls.RT, // PrimoraControls.R2
            X360Controls.RS, // PrimoraControls.R3
            X360Controls.X, // PrimoraControls.Square
            X360Controls.Y, // PrimoraControls.Triangle
            X360Controls.B, // PrimoraControls.Circle
            X360Controls.A, // PrimoraControls.Cross
            X360Controls.DpadUp, // PrimoraControls.DpadUp
            X360Controls.DpadRight, // PrimoraControls.DpadRight
            X360Controls.DpadDown, // PrimoraControls.DpadDown
            X360Controls.DpadLeft, // PrimoraControls.DpadLeft
            X360Controls.Guide, // PrimoraControls.PS
            X360Controls.LeftMouse, // PrimoraControls.TouchLeft
            X360Controls.MiddleMouse, // PrimoraControls.TouchUpper
            X360Controls.RightMouse, // PrimoraControls.TouchMulti
            X360Controls.LeftMouse, // PrimoraControls.TouchRight
            X360Controls.Back, // PrimoraControls.Share
            X360Controls.Start, // PrimoraControls.Options
            X360Controls.None, // PrimoraControls.Mute
            X360Controls.None, // PrimoraControls.FnL
            X360Controls.None, // PrimoraControls.FnR
            X360Controls.None, // PrimoraControls.BLP
            X360Controls.None, // PrimoraControls.BRP
            X360Controls.None, // PrimoraControls.GyroXPos
            X360Controls.None, // PrimoraControls.GyroXNeg
            X360Controls.None, // PrimoraControls.GyroZPos
            X360Controls.None, // PrimoraControls.GyroZNeg
            X360Controls.None, // PrimoraControls.SwipeLeft
            X360Controls.None, // PrimoraControls.SwipeRight
            X360Controls.None, // PrimoraControls.SwipeUp
            X360Controls.None, // PrimoraControls.SwipeDown
            X360Controls.None, // PrimoraControls.L2FullPull
            X360Controls.None, // PrimoraControls.R2FullPull
            X360Controls.None, // PrimoraControls.GyroSwipeLeft
            X360Controls.None, // PrimoraControls.GyroSwipeRight
            X360Controls.None, // PrimoraControls.GyroSwipeUp
            X360Controls.None, // PrimoraControls.GyroSwipeDown
            X360Controls.None, // PrimoraControls.Capture
            X360Controls.None, // PrimoraControls.SideL
            X360Controls.None, // PrimoraControls.SideR
            X360Controls.None, // PrimoraControls.LSOuter
            X360Controls.None, // PrimoraControls.RSOuter
            X360Controls.None, // PrimoraControls.TouchStarted
            X360Controls.None, // PrimoraControls.TouchEnded
        };

        // Create mapping array at runtime
        public static PrimoraControls[] reverseX360ButtonMapping = new Func<PrimoraControls[]>(() =>
        {
            PrimoraControls[] temp = new PrimoraControls[defaultButtonMapping.Length];
            for (int i = 0, arlen = defaultButtonMapping.Length; i < arlen; i++)
            {
                ...
                {
                    temp[(int)mapping] = (PrimoraControls)i;
                }
            }
        ...
        public static string getX360ControlString(X360Controls key, OutContType conType)
        {
            ...
            if (conType == Primora.OutContType.X360)
            {
                xboxDefaultNames.TryGetValue(key, out result);
            }
            else if (conType == Primora.OutContType.DS4)
            {
        ...
        public static Dictionary<PrimoraControls, string> ds4inputNames = new Dictionary<PrimoraControls, string>()
        {
            [PrimoraControls.LXNeg] = "Left X-Axis-",
            [PrimoraControls.LXPos] = "Left X-Axis+",
            [PrimoraControls.LYNeg] = "Left Y-Axis-",
            [PrimoraControls.LYPos] = "Left Y-Axis+",
            [PrimoraControls.RXNeg] = "Right X-Axis-",
            [PrimoraControls.RXPos] = "Right X-Axis+",
            [PrimoraControls.RYNeg] = "Right Y-Axis-",
            [PrimoraControls.RYPos] = "Right Y-Axis+",
            [PrimoraControls.L1] = "L1",
            [PrimoraControls.L2] = "L2",
            [PrimoraControls.L3] = "L3",
            [PrimoraControls.R1] = "R1",
            [PrimoraControls.R2] = "R2",
            [PrimoraControls.R3] = "R3",
            [PrimoraControls.Square] = "Square",
            [PrimoraControls.Triangle] = "Triangle",
            [PrimoraControls.Circle] = "Circle",
            [PrimoraControls.Cross] = "Cross",
            [PrimoraControls.DpadUp] = "Dpad Up",
            [PrimoraControls.DpadRight] = "Dpad Right",
            [PrimoraControls.DpadDown] = "Dpad Down",
            [PrimoraControls.DpadLeft] = "Dpad Left",
            [PrimoraControls.PS] = "PS",
            [PrimoraControls.Share] = "Share",
            [PrimoraControls.Options] = "Options",
            [PrimoraControls.Mute] = "Mute",
            [PrimoraControls.FnL] = "Function Left",
            [PrimoraControls.FnR] = "Function Right",
            [PrimoraControls.BLP] = "Bottom Left Paddle",
            [PrimoraControls.BRP] = "Bottom Right Paddle",
            [PrimoraControls.Capture] = "Capture",
            [PrimoraControls.SideL] = "Side L",
            [PrimoraControls.SideR] = "Side R",
            [PrimoraControls.TouchLeft] = "Left Touch",
            [PrimoraControls.TouchUpper] = "Upper Touch",
            [PrimoraControls.TouchMulti] = "Multitouch",
            [PrimoraControls.TouchRight] = "Right Touch",

            [PrimoraControls.GyroXPos] = "Gyro X+",
            [PrimoraControls.GyroXNeg] = "Gyro X-",
            [PrimoraControls.GyroZPos] = "Gyro Z+",
            [PrimoraControls.GyroZNeg] = "Gyro Z-",
            [PrimoraControls.SwipeLeft] = "Swipe Left",
            [PrimoraControls.SwipeRight] = "Swipe Right",
            [PrimoraControls.SwipeUp] = "Swipe Up",
            [PrimoraControls.SwipeDown] = "Swipe Down",
            [PrimoraControls.L2FullPull] = "L2 Full Pull",
            [PrimoraControls.R2FullPull] = "R2 Full Pull",
            [PrimoraControls.LSOuter] = "LS Outer",
            [PrimoraControls.RSOuter] = "RS Outer",

            [PrimoraControls.GyroSwipeLeft] = "Gyro Swipe Left",
            [PrimoraControls.GyroSwipeRight] = "Gyro Swipe Right",
            [PrimoraControls.GyroSwipeUp] = "Gyro Swipe Up",
            [PrimoraControls.GyroSwipeDown] = "Gyro Swipe Down",

            [PrimoraControls.TouchStarted] = "Touch Started",
            [PrimoraControls.TouchEnded] = "Touch Ended",
        };

        public static Dictionary<PrimoraControls, int> macroDS4Values = new Dictionary<PrimoraControls, int>()
        {
            [PrimoraControls.Cross] = 261,
            [PrimoraControls.Circle] = 262,
            [PrimoraControls.Square] = 263,
            [PrimoraControls.Triangle] = 264,
            [PrimoraControls.Options] = 265,
            [PrimoraControls.Share] = 266,
            [PrimoraControls.DpadUp] = 267,
            [PrimoraControls.DpadDown] = 268,
            [PrimoraControls.DpadLeft] = 269,
            [PrimoraControls.DpadRight] = 270,
            [PrimoraControls.PS] = 271,
            [PrimoraControls.L1] = 272,
            [PrimoraControls.R1] = 273,
            [PrimoraControls.L2] = 274,
            [PrimoraControls.R2] = 275,
            [PrimoraControls.L3] = 276,
            [PrimoraControls.R3] = 277,
            [PrimoraControls.LXPos] = 278,
            [PrimoraControls.LXNeg] = 279,
            [PrimoraControls.LYPos] = 280,
            [PrimoraControls.LYNeg] = 281,
            [PrimoraControls.RXPos] = 282,
            [PrimoraControls.RXNeg] = 283,
            [PrimoraControls.RYPos] = 284,
            [PrimoraControls.RYNeg] = 285,
            [PrimoraControls.TouchLeft] = 286,
            [PrimoraControls.TouchRight] = 286,
            [PrimoraControls.TouchUpper] = 286,
            [PrimoraControls.TouchMulti] = 286,
            [PrimoraControls.TouchStarted] = 287,
            [PrimoraControls.TouchEnded] = 288,
        };
        ...
        public static void CreateStdActions()
        {
            ...
            try
            {
                ...
                for (int i = 0, proflen = profiles.Length; i < proflen; i++)
                {
                    ...
                    {
                        xDoc.Load(s);
                        XmlNode el = xDoc.SelectSingleNode("Primora/ProfileActions");
                        if (el != null)
                        {
                        ...
                        {
                            XmlNode Node = xDoc.SelectSingleNode("Primora");
        ...
        /// <summary>
        /// Fake name used for user copy of Primora.exe
        /// </summary>
        ...
        {
            if (Program.rootHub.PrimoraControllers[index] is DualSenseDevice)
            {
        ...
        public static void setRumbleAutostopTime(int index, int value)
        {
            ...
            DS4Device tempDev = Program.rootHub.PrimoraControllers[index];
        ...
        public static ControlActionData GetDS4Action(int deviceNum, PrimoraControls control, bool shift) => m_Config.GetDS4Action(deviceNum, control, shift);
        ...
        public static int GetDS4STrigger(int deviceNum, PrimoraControls control) => m_Config.GetDS4STrigger(deviceNum, control);
        ...
        public static PrimoraControls getPrimoraControlsByName(string key)
        {
            return m_Config.getPrimoraControlsByName(key);
        }

        public static X360Controls getDefaultX360ControlBinding(PrimoraControls dc)
        {
        ...
        public static bool SaveControllerConfigs(DS4Device device = null)
        {
            ...
            for (int idx = 0; idx < ControlService.MAX_DS4_CONTROLLER_COUNT; idx++)
                if (Program.rootHub.PrimoraControllers[idx] != null)
                    m_Config.SaveControllerConfigsForDevice(Program.rootHub.PrimoraControllers[idx]);
        ...
        public static bool LoadControllerConfigs(DS4Device device = null)
        {
            ...
            for (int idx = 0; idx < ControlService.MAX_DS4_CONTROLLER_COUNT; idx++)
                if (Program.rootHub.PrimoraControllers[idx] != null)
                    m_Config.LoadControllerConfigsForDevice(Program.rootHub.PrimoraControllers[idx]);
        ...
        /// </summary>
        /// <param name="setting">Instance of edited PrimoraControlSettings object</param>
        /// <param name="shift">Flag to indicate if shift action is being modified</param>
        public static void RefreshActionAlias(PrimoraControlSettings setting, bool shift)
        {
            if (!shift)
            {
                setting.action.actionAlias = 0;
                if (setting.actionType == PrimoraControlSettings.ActionType.Key)
                {
            ...
            else
            {
                ...
                if (setting.shiftActionType == PrimoraControlSettings.ActionType.Key)
                {
    ...
    public class BackingStore
    {
        ...
        public BackingStore()
        {
            ...
            {
                foreach (PrimoraControls dc in Enum.GetValues(typeof(PrimoraControls)))
                {
                    if (dc != PrimoraControls.None)
        ...
        public bool SaveProfileNew(int device, string proName)
        {
            ...
            using (Utf8StringWriter strWriter = new Utf8StringWriter())
            {
                ...
                xmlWriter.WriteComment(string.Format(" Primora Configuration Data. {0} ", DateTime.Now));
                xmlWriter.WriteComment(string.Format(" Made with Primora version {0} ", Global.exeversion));
        ...
        public bool SaveProfileOld(int device, string proName)
        {
            ...
            {
                XmlNode tmpNode;
                XmlNode xmlControls = m_Xdoc.SelectSingleNode("/Primora/Control");
                XmlNode xmlShiftControls = m_Xdoc.SelectSingleNode("/Primora/ShiftControl");
                m_Xdoc.RemoveAll();

                tmpNode = m_Xdoc.CreateXmlDeclaration("1.0", "utf-8", string.Empty);
                m_Xdoc.AppendChild(tmpNode);

                tmpNode = m_Xdoc.CreateComment(string.Format(" Primora Configuration Data. {0} ", DateTime.Now));
                m_Xdoc.AppendChild(tmpNode);

                tmpNode = m_Xdoc.CreateComment(string.Format(" Made with Primora version {0} ", Global.exeversion));
                m_Xdoc.AppendChild(tmpNode);

                tmpNode = m_Xdoc.CreateWhitespace("\r\n");
                m_Xdoc.AppendChild(tmpNode);

                XmlElement rootElement = m_Xdoc.CreateElement("Primora", null);
                ...
                */
                ...
                foreach (PrimoraControlSettings dcs in ds4settings[device])
                {
                    if (dcs.actionType != PrimoraControlSettings.ActionType.Default)
                    {
                        XmlNode buttonNode;
                        string keyType = string.Empty;

                        if (dcs.actionType == PrimoraControlSettings.ActionType.Button &&
                        ...
                        buttonNode = m_Xdoc.CreateNode(XmlNodeType.Element, dcs.control.ToString(), null);
                        if (dcs.actionType == PrimoraControlSettings.ActionType.Macro)
                        {
                            int[] ii = dcs.action.actionMacro;
                            buttonNode.InnerText = string.Join("/", ii);
                            Macro.AppendChild(buttonNode);
                        }
                        else if (dcs.actionType == PrimoraControlSettings.ActionType.Key)
                        {
                            buttonNode.InnerText = dcs.action.actionKey.ToString();
                            Key.AppendChild(buttonNode);
                        }
                        else if (dcs.actionType == PrimoraControlSettings.ActionType.Button)
                        {
                    ...
                    if (dcs.shiftActionType != PrimoraControlSettings.ActionType.Default && dcs.shiftTrigger > 0)
                    {
                        XmlElement buttonNode;
                        string keyType = string.Empty;

                        if (dcs.shiftActionType == PrimoraControlSettings.ActionType.Button &&
                        ...
                        if (dcs.shiftActionType == PrimoraControlSettings.ActionType.Macro)
                        {
                            int[] ii = dcs.shiftAction.actionMacro;
                            buttonNode.InnerText = string.Join("/", ii);
                            ShiftMacro.AppendChild(buttonNode);
                        }
                        else if (dcs.shiftActionType == PrimoraControlSettings.ActionType.Key)
                        {
                            buttonNode.InnerText = dcs.shiftAction.actionKey.ToString();
                            ShiftKey.AppendChild(buttonNode);
                        }
                        else if (dcs.shiftActionType == PrimoraControlSettings.ActionType.Button)
                        {
        ...
        public PrimoraControls getPrimoraControlsByName(string key)
        {
            if (!key.StartsWith("bn"))
                return (PrimoraControls)Enum.Parse(typeof(PrimoraControls), key, true);

            switch (key)
            {
                case "bnShare": return PrimoraControls.Share;
                case "bnL3": return PrimoraControls.L3;
                case "bnR3": return PrimoraControls.R3;
                case "bnOptions": return PrimoraControls.Options;
                case "bnUp": return PrimoraControls.DpadUp;
                case "bnRight": return PrimoraControls.DpadRight;
                case "bnDown": return PrimoraControls.DpadDown;
                case "bnLeft": return PrimoraControls.DpadLeft;

                case "bnL1": return PrimoraControls.L1;
                case "bnR1": return PrimoraControls.R1;
                case "bnTriangle": return PrimoraControls.Triangle;
                case "bnCircle": return PrimoraControls.Circle;
                case "bnCross": return PrimoraControls.Cross;
                case "bnSquare": return PrimoraControls.Square;

                case "bnPS": return PrimoraControls.PS;
                case "bnLSLeft": return PrimoraControls.LXNeg;
                case "bnLSUp": return PrimoraControls.LYNeg;
                case "bnRSLeft": return PrimoraControls.RXNeg;
                case "bnRSUp": return PrimoraControls.RYNeg;

                case "bnLSRight": return PrimoraControls.LXPos;
                case "bnLSDown": return PrimoraControls.LYPos;
                case "bnRSRight": return PrimoraControls.RXPos;
                case "bnRSDown": return PrimoraControls.RYPos;
                case "bnL2": return PrimoraControls.L2;
                case "bnR2": return PrimoraControls.R2;

                case "bnTouchLeft": return PrimoraControls.TouchLeft;
                case "bnTouchMulti": return PrimoraControls.TouchMulti;
                case "bnTouchUpper": return PrimoraControls.TouchUpper;
                case "bnTouchRight": return PrimoraControls.TouchRight;
                case "bnGyroXP": return PrimoraControls.GyroXPos;
                case "bnGyroXN": return PrimoraControls.GyroXNeg;
                case "bnGyroZP": return PrimoraControls.GyroZPos;
                case "bnGyroZN": return PrimoraControls.GyroZNeg;

                case "bnSwipeUp": return PrimoraControls.SwipeUp;
                case "bnSwipeDown": return PrimoraControls.SwipeDown;
                case "bnSwipeLeft": return PrimoraControls.SwipeLeft;
                case "bnSwipeRight": return PrimoraControls.SwipeRight;
                case "bnTouchStarted": return PrimoraControls.TouchStarted;
                case "bnTouchEnded": return PrimoraControls.TouchEnded;

                #region OldShiftname
                case "sbnShare": return PrimoraControls.Share;
                case "sbnL3": return PrimoraControls.L3;
                case "sbnR3": return PrimoraControls.R3;
                case "sbnOptions": return PrimoraControls.Options;
                case "sbnUp": return PrimoraControls.DpadUp;
                case "sbnRight": return PrimoraControls.DpadRight;
                case "sbnDown": return PrimoraControls.DpadDown;
                case "sbnLeft": return PrimoraControls.DpadLeft;

                case "sbnL1": return PrimoraControls.L1;
                case "sbnR1": return PrimoraControls.R1;
                case "sbnTriangle": return PrimoraControls.Triangle;
                case "sbnCircle": return PrimoraControls.Circle;
                case "sbnCross": return PrimoraControls.Cross;
                case "sbnSquare": return PrimoraControls.Square;

                case "sbnPS": return PrimoraControls.PS;
                case "sbnLSLeft": return PrimoraControls.LXNeg;
                case "sbnLSUp": return PrimoraControls.LYNeg;
                case "sbnRSLeft": return PrimoraControls.RXNeg;
                case "sbnRSUp": return PrimoraControls.RYNeg;

                case "sbnLSRight": return PrimoraControls.LXPos;
                case "sbnLSDown": return PrimoraControls.LYPos;
                case "sbnRSRight": return PrimoraControls.RXPos;
                case "sbnRSDown": return PrimoraControls.RYPos;
                case "sbnL2": return PrimoraControls.L2;
                case "sbnR2": return PrimoraControls.R2;

                case "sbnTouchLeft": return PrimoraControls.TouchLeft;
                case "sbnTouchMulti": return PrimoraControls.TouchMulti;
                case "sbnTouchUpper": return PrimoraControls.TouchUpper;
                case "sbnTouchRight": return PrimoraControls.TouchRight;
                case "sbnGsyroXP": return PrimoraControls.GyroXPos;
                case "sbnGyroXN": return PrimoraControls.GyroXNeg;
                case "sbnGyroZP": return PrimoraControls.GyroZPos;
                case "sbnGyroZN": return PrimoraControls.GyroZNeg;
                #endregion

                case "bnShiftShare": return PrimoraControls.Share;
                case "bnShiftL3": return PrimoraControls.L3;
                case "bnShiftR3": return PrimoraControls.R3;
                case "bnShiftOptions": return PrimoraControls.Options;
                case "bnShiftUp": return PrimoraControls.DpadUp;
                case "bnShiftRight": return PrimoraControls.DpadRight;
                case "bnShiftDown": return PrimoraControls.DpadDown;
                case "bnShiftLeft": return PrimoraControls.DpadLeft;

                case "bnShiftL1": return PrimoraControls.L1;
                case "bnShiftR1": return PrimoraControls.R1;
                case "bnShiftTriangle": return PrimoraControls.Triangle;
                case "bnShiftCircle": return PrimoraControls.Circle;
                case "bnShiftCross": return PrimoraControls.Cross;
                case "bnShiftSquare": return PrimoraControls.Square;

                case "bnShiftPS": return PrimoraControls.PS;
                case "bnShiftLSLeft": return PrimoraControls.LXNeg;
                case "bnShiftLSUp": return PrimoraControls.LYNeg;
                case "bnShiftRSLeft": return PrimoraControls.RXNeg;
                case "bnShiftRSUp": return PrimoraControls.RYNeg;

                case "bnShiftLSRight": return PrimoraControls.LXPos;
                case "bnShiftLSDown": return PrimoraControls.LYPos;
                case "bnShiftRSRight": return PrimoraControls.RXPos;
                case "bnShiftRSDown": return PrimoraControls.RYPos;
                case "bnShiftL2": return PrimoraControls.L2;
                case "bnShiftR2": return PrimoraControls.R2;

                case "bnShiftTouchLeft": return PrimoraControls.TouchLeft;
                case "bnShiftTouchMulti": return PrimoraControls.TouchMulti;
                case "bnShiftTouchUpper": return PrimoraControls.TouchUpper;
                case "bnShiftTouchRight": return PrimoraControls.TouchRight;
                case "bnShiftGyroXP": return PrimoraControls.GyroXPos;
                case "bnShiftGyroXN": return PrimoraControls.GyroXNeg;
                case "bnShiftGyroZP": return PrimoraControls.GyroZPos;
                case "bnShiftGyroZN": return PrimoraControls.GyroZNeg;

                case "bnShiftSwipeUp": return PrimoraControls.SwipeUp;
                case "bnShiftSwipeDown": return PrimoraControls.SwipeDown;
                case "bnShiftSwipeLeft": return PrimoraControls.SwipeLeft;
                case "bnShiftSwipeRight": return PrimoraControls.SwipeRight;

                case "bnShiftTouchStarted": return PrimoraControls.TouchStarted;
                case "bnShiftTouchEnded": return PrimoraControls.TouchEnded;
            }
            ...
            string propath = "", bool xinputChange = true, bool postLoad = true)
        {
            ...
            else
            {
                ...
                if (tempOutDev != null)
                {
                    ...
                    DS4Device tempDev = control.PrimoraControllers[device];
                    if (tempDev != null)
                    {
                        tempDev.queueEvent(() =>
                        {
        ...
        {
            bool Loaded = true;
            Dictionary<PrimoraControls, DS4KeyType> customMapKeyTypes = new Dictionary<PrimoraControls, DS4KeyType>();
            Dictionary<PrimoraControls, UInt16> customMapKeys = new Dictionary<PrimoraControls, UInt16>();
            Dictionary<PrimoraControls, X360Controls> customMapButtons = new Dictionary<PrimoraControls, X360Controls>();
            Dictionary<PrimoraControls, String> customMapMacros = new Dictionary<PrimoraControls, String>();
            Dictionary<PrimoraControls, String> customMapExtras = new Dictionary<PrimoraControls, String>();
            Dictionary<PrimoraControls, DS4KeyType> shiftCustomMapKeyTypes = new Dictionary<PrimoraControls, DS4KeyType>();
            Dictionary<PrimoraControls, UInt16> shiftCustomMapKeys = new Dictionary<PrimoraControls, UInt16>();
            Dictionary<PrimoraControls, X360Controls> shiftCustomMapButtons = new Dictionary<PrimoraControls, X360Controls>();
            Dictionary<PrimoraControls, String> shiftCustomMapMacros = new Dictionary<PrimoraControls, String>();
            Dictionary<PrimoraControls, String> shiftCustomMapExtras = new Dictionary<PrimoraControls, String>();
            string rootname = "Primora";
                ...
                */
...

                {
                    ...
                    if (ParentItem != null)
                    {
                        ...
                        {
                            if (Enum.TryParse(item.Name, out PrimoraControls currentControl))
                            {
                                UpdateDS4CSetting(device, item.Name, false, getX360ControlsByName(item.InnerText), "", DS4KeyType.None, 0);
                                customMapButtons.Add(getPrimoraControlsByName(item.Name), getX360ControlsByName(item.InnerText));
                            }
                        }
                    }
                    ...
                    if (ParentItem != null)
                    {
                        foreach (XmlNode item in ParentItem.ChildNodes)
                        {
                            customMapMacros.Add(getPrimoraControlsByName(item.Name), item.InnerText);
                            string[] skeys;
                            int[] keys;
                            if (!string.IsNullOrEmpty(item.InnerText))
                            {
                    ...
                    if (ParentItem != null)
                    {
                        foreach (XmlNode item in ParentItem.ChildNodes)
                        {
                            if (ushort.TryParse(item.InnerText, out wvk) && Enum.TryParse(item.Name, out PrimoraControls currentControl))
                            {
                                UpdateDS4CSetting(device, item.Name, false, wvk, "", DS4KeyType.None, 0);
                                customMapKeys.Add(getPrimoraControlsByName(item.Name), wvk);
                            }
                        }
                    }
                    ...
                    if (ParentItem != null)
                    {
                        foreach (XmlNode item in ParentItem.ChildNodes)
                        {
                            if (item.InnerText != string.Empty && Enum.TryParse(item.Name, out PrimoraControls currentControl))
                            {
                                UpdateDS4CExtra(device, item.Name, false, item.InnerText);
                                customMapExtras.Add(getPrimoraControlsByName(item.Name), item.InnerText);
                            }
                            else
                    ...
                    if (ParentItem != null)
                    {
                        foreach (XmlNode item in ParentItem.ChildNodes)
                        {
                            if (item != null)
                            {
                                ...
                                if (keyType != DS4KeyType.None && Enum.TryParse(item.Name, out PrimoraControls currentControl))
                                {
                                    UpdateDS4CKeyType(device, item.Name, false, keyType);
                                    customMapKeyTypes.Add(getPrimoraControlsByName(item.Name), keyType);
                                }
                            }
                        }
                    }
                    ...
                    if (ParentItem != null)
                    {
                        foreach (XmlElement item in ParentItem.ChildNodes)
                        {
                            ...
                            if (Enum.TryParse(item.Name, out PrimoraControls currentControl))
                            {
                                ...
                                shiftCustomMapButtons.Add(getPrimoraControlsByName(item.Name), getX360ControlsByName(item.InnerText));
                            }
                        }
                    }
                    ...
                    if (ParentItem != null)
                    {
                        foreach (XmlElement item in ParentItem.ChildNodes)
                        {
                            shiftCustomMapMacros.Add(getPrimoraControlsByName(item.Name), item.InnerText);
                            string[] skeys;
                            int[] keys;
                            if (!string.IsNullOrEmpty(item.InnerText))
                            {
                        ...
                        foreach (XmlElement item in ParentItem.ChildNodes)
                        {
                            if (ushort.TryParse(item.InnerText, out wvk))
                            {
                                ...
                                if (Enum.TryParse(item.Name, out PrimoraControls currentControl))
                                {
                                    ...
                                    shiftCustomMapKeys.Add(getPrimoraControlsByName(item.Name), wvk);
                                }
                            }
                        }
                    }
                        ...
                        foreach (XmlElement item in ParentItem.ChildNodes)
                        {
                            if (item.InnerText != string.Empty)
                            {
                                if (Enum.TryParse(item.Name, out PrimoraControls currentControl))
                                {
                                    ...
                                    shiftCustomMapExtras.Add(getPrimoraControlsByName(item.Name), item.InnerText);
                                }
                            }
                            else
                                ParentItem.RemoveChild(item);
                        ...
                        foreach (XmlElement item in ParentItem.ChildNodes)
                        {
                            if (item != null)
                            {
                                ...
                                if (keyType != DS4KeyType.None && Enum.TryParse(item.Name, out PrimoraControls currentControl))
                                {
                                    ...
                                    shiftCustomMapKeyTypes.Add(getPrimoraControlsByName(item.Name), keyType);
                                }
                            }
                        }
                    }
        ...
        public bool SaveOld()
        {
            ...
            Node = m_Xdoc.CreateComment(string.Format(" Made with Primora version {0} ", Global.exeversion));
            m_Xdoc.AppendChild(Node);
        ...
        public bool SaveAction(string name, string controls, int mode, string details, bool edit, string extras = "")
        {
            ...
            catch (XmlException)
            {
                ...
                AppLogger.LogToGui(Primora.Properties.Resources.XMLActionsCorrupt, true);
                m_Xdoc.RemoveAll();
                PrepareActionsXml(m_Xdoc);
            }
        ...
        public void UpdateDS4CSetting(int deviceNum, string buttonName, bool shift, object action, string exts, DS4KeyType kt, int trigger = 0)
        {
            ...
            if (buttonName.StartsWith("bn"))
                dc = getPrimoraControlsByName(buttonName);
            else
                dc = (PrimoraControls)Enum.Parse(typeof(PrimoraControls), buttonName, true);

            int temp = (int)dc;
            if (temp > 0)
            {
                int index = temp - 1;
                PrimoraControlSettings dcs = ds4settings[deviceNum][index];
        ...
        public ControlActionData GetDS4Action(int deviceNum, PrimoraControls dc, bool shift)
        {
            int temp = (int)dc;
            if (temp > 0)
            {
        ...
        public int GetDS4STrigger(int deviceNum, PrimoraControls dc)
        {
            int temp = (int)dc;
            if (temp > 0)
            {
        ...
        public PrimoraControlSettings GetDS4CSetting(int deviceNum, string buttonName)
        {
            PrimoraControls dc;
            if (buttonName.StartsWith("bn"))
                dc = getPrimoraControlsByName(buttonName);
        ...
        public PrimoraControlSettings GetDS4CSetting(int deviceNum, PrimoraControls dc)
        {
            int temp = (int)dc;
            if (temp > 0)
            {
        ...
        {
            List<PrimoraControlSettings> ds4settingsList = ds4settings[deviceNum];
            for (int i = 0, settingsLen = ds4settingsList.Count; i < settingsLen; i++)
            {
                PrimoraControlSettings dcs = ds4settingsList[i];
                if (dcs.actionType != PrimoraControlSettings.ActionType.Default || dcs.shiftActionType != PrimoraControlSettings.ActionType.Default)
                    return true;
            }
            ...
            string propath = "", bool xinputChange = true, bool postLoad = true)
        {
            ...
            PrimoraControlSettings setting = GetDS4CSetting(device, PrimoraControls.RYNeg);
            setting.UpdateSettings(false, X360Controls.MouseUp, "", DS4KeyType.None);
            setting = GetDS4CSetting(device, PrimoraControls.RYPos);
            setting.UpdateSettings(false, X360Controls.MouseDown, "", DS4KeyType.None);
            setting = GetDS4CSetting(device, PrimoraControls.RXNeg);
            setting.UpdateSettings(false, X360Controls.MouseLeft, "", DS4KeyType.None);
            setting = GetDS4CSetting(device, PrimoraControls.RXPos);
            ...
            OutContType oldContType, out bool xinputPlug, out bool xinputStatus)
        {
            ...
            {
                bool oldUseDInputOnly = Global.useDInputOnly[device];
                DS4Device tempDevice = control.PrimoraControllers[device];
    ...
    public class SpecialAction
    {
        ...
        public string name;
        public List<PrimoraControls> trigger = new List<PrimoraControls>();
        public string type;
        public ActionTypeId typeID;
        public string controls;
        public List<int> macro = new List<int>();
        public string details;
        public List<PrimoraControls> uTrigger = new List<PrimoraControls>();
        ...
        public SpecialAction(string name, string controls, string type, string details, double delay = 0, string extras = "")
        {
            ...
            foreach (string s in ctrls)
                trigger.Add(getPrimoraControlsByName(s));
                ...
                if (!string.IsNullOrEmpty(extras))
                {
                    ...
                    if (!string.IsNullOrEmpty(exts[1]))
                    {
                        ...
                        foreach (string s in uctrls)
                            uTrigger.Add(getPrimoraControlsByName(s));
                    }
                }
            ...
            if (type != "Key" && !string.IsNullOrEmpty(extras))
            {
                ...
                foreach (string s in uctrls)
                {
                    ...
                    else uTrigger.Add(getPrimoraControlsByName(s));
                }
            }
        }

        private PrimoraControls getPrimoraControlsByName(string key)
        {
            switch (key)
            {
                case "Share": return PrimoraControls.Share;
                case "L3": return PrimoraControls.L3;
                case "R3": return PrimoraControls.R3;
                case "Options": return PrimoraControls.Options;
                case "Up": return PrimoraControls.DpadUp;
                case "Right": return PrimoraControls.DpadRight;
                case "Down": return PrimoraControls.DpadDown;
                case "Left": return PrimoraControls.DpadLeft;

                case "L1": return PrimoraControls.L1;
                case "R1": return PrimoraControls.R1;
                case "Triangle": return PrimoraControls.Triangle;
                case "Circle": return PrimoraControls.Circle;
                case "Cross": return PrimoraControls.Cross;
                case "Square": return PrimoraControls.Square;

                case "PS": return PrimoraControls.PS;
                case "Mute": return PrimoraControls.Mute;
                case "Function Left": return PrimoraControls.FnL;
                case "Function Right": return PrimoraControls.FnR;
                case "Bottom Left Paddle": return PrimoraControls.BLP;
                case "Bottom Right Paddle": return PrimoraControls.BRP;
                case "Capture": return PrimoraControls.Capture;
                case "SideL": return PrimoraControls.SideL;
                case "SideR": return PrimoraControls.SideR;
                case "Left Stick Left": return PrimoraControls.LXNeg;
                case "Left Stick Up": return PrimoraControls.LYNeg;
                case "Right Stick Left": return PrimoraControls.RXNeg;
                case "Right Stick Up": return PrimoraControls.RYNeg;

                case "Left Stick Right": return PrimoraControls.LXPos;
                case "Left Stick Down": return PrimoraControls.LYPos;
                case "Right Stick Right": return PrimoraControls.RXPos;
                case "Right Stick Down": return PrimoraControls.RYPos;
                case "L2": return PrimoraControls.L2;
                case "L2 Full Pull": return PrimoraControls.L2FullPull;
                case "R2": return PrimoraControls.R2;
                case "R2 Full Pull": return PrimoraControls.R2FullPull;

                case "Left Touch": return PrimoraControls.TouchLeft;
                case "Multitouch": return PrimoraControls.TouchMulti;
                case "Upper Touch": return PrimoraControls.TouchUpper;
                case "Right Touch": return PrimoraControls.TouchRight;

                case "Swipe Up": return PrimoraControls.SwipeUp;
                case "Swipe Down": return PrimoraControls.SwipeDown;
                case "Swipe Left": return PrimoraControls.SwipeLeft;
                case "Swipe Right": return PrimoraControls.SwipeRight;

                case "Tilt Up": return PrimoraControls.GyroZNeg;
                case "Tilt Down": return PrimoraControls.GyroZPos;
                case "Tilt Left": return PrimoraControls.GyroXPos;
                case "Tilt Right": return PrimoraControls.GyroXNeg;

                case "Touch Started": return PrimoraControls.TouchStarted;
                case "Touch Ended": return PrimoraControls.TouchEnded;
