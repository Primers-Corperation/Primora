...
//
// This module is a feeder for VJoy virtual joystick driver. Primora can optionally re-map and feed buttons and analog axis values from DS4 Controller to VJoy device.
// At first this may seem silly because Primora can already do re-mapping by using a virtual X360 Controller driver, so why feed VJoy virtual driver also? 
// Sometimes X360 driver may run out of analog axis options, so for example "SA motion sensor steering wheel emulation" in Primora would reserve a thumbstick X or Y 
...
// The problem can be solved by configuring Primora to re-map SA steering wheel emulation axis to VJoy axis, so all analog axies in DS4 controller are still available for normal usage.
//
    ...
    public class vJoyFeeder
    {
        ...
        public static void InitializeVJoyDevice(uint vJoyID, HID_USAGES axis)
        {
            lock (vJoyLocker)
            {
                ...
                {
                    vJoyAvailable[vJoyID - 1] = false;
                    AppLogger.LogToGui("ERROR. vJoy initialization failed. Make sure that Primora application can find vJoyInterface.dll library file", false);
                }
            }
        }

        // Feed axis value to VJoy virtual joystic driver (Primora sixaxis (SA) motion sensor steering wheel emulation feature can optionally feed VJoy analog axis instead of ScpVBus x360 axis
