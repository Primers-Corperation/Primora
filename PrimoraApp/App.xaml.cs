...
namespace Primora
{
    ...
    public partial class App : Application
    {
        ...
        public static Primora.ControlService rootHub;
        ...
        private MemoryMappedFile ipcResultDataMMF = null; // MemoryMappedFile for inter-process communication used to exchange string result data between cmdline client process and the background running Primora app

        private static Dictionary<Primora.AppThemeChoice, string> themeLocs = new
            Dictionary<Primora.AppThemeChoice, string>()
        {
            [Primora.AppThemeChoice.Default] = "PrimoraForms/Themes/DefaultTheme.xaml",
            [Primora.AppThemeChoice.Light] = "PrimoraForms/Themes/DefaultTheme.xaml",
            [Primora.AppThemeChoice.Dark] = "PrimoraForms/Themes/DarkTheme.xaml",
        };
        ...
        private void Application_Startup(object sender, StartupEventArgs e)
        {
            ...
            Primora.Util.NtSetInformationProcess(Process.GetCurrentProcess().Handle,
                Primora.Util.PROCESS_INFORMATION_CLASS.ProcessIoPriority, ref ioPrio, 4);

            // Force Normal Page Priority
            IntPtr pagePrio = new IntPtr(5);
            Primora.Util.NtSetInformationProcess(Process.GetCurrentProcess().Handle,
                Primora.Util.PROCESS_INFORMATION_CLASS.ProcessPagePriority, ref pagePrio, 4);
            ...
            Primora.Util.timeBeginPeriod(1);

            // Retrieve info about installed ViGEmBus device if found
            Primora.Global.RefreshViGEmBusInfo();
            ...
            Primora.Global.FindConfigLocation();
            bool firstRun = Primora.Global.firstRun;

            // Could not find unique profile location; does not exist or multiple places.
            // Advise user to specify where Primora should save its configuation files
            // and profiles
            if (firstRun)
            {
                PrimoraForms.SaveWhere savewh =
                    new PrimoraForms.SaveWhere(Primora.Global.multisavespots);
                savewh.ShowDialog();
            ...
            {
                MessageBox.Show($"Cannot create config folder structure in {Primora.Global.appdatapath}. Exiting",
                    "Primora", MessageBoxButton.OK, MessageBoxImage.Error);
                Current.Shutdown(1);
                return;
            }
            ...
            logger.Info($"Primora version {version}");
            logger.Info($"Primora exe file: {Primora.Global.exeFileName}");
            logger.Info($"Primora Assembly Architecture: {(Environment.Is64BitProcess ? "x64" : "x86")}");
            logger.Info($"OS Version: {Environment.OSVersion}");
            logger.Info($"OS Product Name: {Primora.Util.GetOSProductName()}");
            logger.Info($"OS Release ID: {Primora.Util.GetOSReleaseId()}");
            logger.Info($"System Architecture: {(Environment.Is64BitOperatingSystem ? "x64" : "x86")}");
            logger.Info("Logger created");

            bool readAppConfig = Primora.Global.Load();
            if (!firstRun && !readAppConfig)
            {
                logger.Info($@"Profiles.xml not read at location ${Primora.Global.appdatapath}\Profiles.xml. Using default app settings");
            }
            ...
            if (firstRun)
            {
                PrimoraForms.FirstLaunchUtilWindow firstLaunchUtilWin =
                    new PrimoraForms.FirstLaunchUtilWindow(Primora.Global.DeviceOptions);
            ...
            if (firstRun)
            {
                ...
                Primora.Global.SaveAsNewProfile(0, "Default");
                for (int i = 0; i < Primora.ControlService.MAX_DS4_CONTROLLER_COUNT; i++)
                {
                    Primora.Global.ProfilePath[i] = Primora.Global.OlderProfilePath[i] = "Default";
                }
            ...
            if (!Primora.Global.LoadActions())
            {
                Primora.Global.CreateStdActions();
            }

            // Have app use selected culture
            SetUICulture(Primora.Global.UseLang);
            Primora.AppThemeChoice themeChoice = Primora.Global.UseCurrentTheme;
            ChangeTheme(Primora.Global.UseCurrentTheme, false);

            Primora.Global.LoadLinkedProfiles();
            PrimoraForms.MainWindow window = new PrimoraForms.MainWindow(parser);
            MainWindow = window;
            ...
            bool runningAsAdmin = Primora.Global.IsAdministrator();
            rootHub.LogDebug($"Running as {(runningAsAdmin ? "Admin" : "User")}");

            if (Primora.Global.hidHideInstalled)
            {
        ...
        private bool CreateConfDirSkeleton()
        {
            ...
            try
            {
                Directory.CreateDirectory(Primora.Global.appdatapath);
                Directory.CreateDirectory(Primora.Global.appdatapath + @"\Profiles\");
                Directory.CreateDirectory(Primora.Global.appdatapath + @"\Logs\");
                //Directory.CreateDirectory(Primora.Global.appdatapath + @"\Macros\");
            }
        ...
        {
            if (!Primora.Global.Save()) //if can't write to file
            {
                if (MessageBox.Show("Cannot write at current location\nCopy Settings to appdata?", "Primora",
                    MessageBoxButton.YesNo, MessageBoxImage.Warning) == MessageBoxResult.Yes)
                {
                    try
                    {
                        Directory.CreateDirectory(Primora.Global.appDataPpath);
                        File.Copy(Primora.Global.exedirpath + "\\Profiles.xml",
                            Primora.Global.appDataPpath + "\\Profiles.xml");
                        File.Copy(Primora.Global.exedirpath + "\\Auto Profiles.xml",
                            Primora.Global.appDataPpath + "\\Auto Profiles.xml");
                        Directory.CreateDirectory(Primora.Global.appDataPpath + "\\Profiles");
                        foreach (string s in Directory.GetFiles(Primora.Global.exedirpath + "\\Profiles"))
                        {
                            File.Copy(s, Primora.Global.appDataPpath + "\\Profiles\\" + Path.GetFileName(s));
                        }
                    }
                    catch { }
                    MessageBox.Show("Copy complete, please relaunch Primora and remove settings from Program Directory",
                        "Primora");
                }
                else
                {
                    MessageBox.Show("Primora cannot edit settings here, This will now close",
                        "Primora");
                }

                Primora.Global.appdatapath = null;
                skipSave = true;
                Current.Shutdown();
                return;
            }
        ...
        private void CheckOptions(ArgumentParser parser)
        {
            ...
            else if (parser.Driverinstall)
            {
                ...
                // Load Primora config if it exists
                Primora.Global.FindConfigLocation();
                bool readAppConfig = Primora.Global.Load();
                if (readAppConfig)
                {
                ...
                CreateBaseThread();
                PrimoraForms.WelcomeDialog dialog = new PrimoraForms.WelcomeDialog(true);
                dialog.ShowDialog();
                runShutdown = false;
                exitApp = true;
                Current.Shutdown();
            ...
            {
                Primora.DS4Devices.reEnableDevice(parser.DeviceInstanceId);
                runShutdown = false;
                exitApp = true;
                Current.Shutdown();
            }
            ...
            {
                IntPtr hWndPrimoraForm = IntPtr.Zero;
                hWndPrimoraForm = FindWindow(ReadIPCClassNameMMF(), "Primora");
                if (hWndPrimoraForm != IntPtr.Zero)
                {
                    ...
                    try
                    {
                        ...
                        {
                            // Query.device# (1..4) command returns a string result via memory mapped file. The cmd is sent to the background Primora 
                            ...
                            ipcSingleTaskMutex = new Mutex(false, "Primora_IPCResultData_SingleTaskMtx");
                            try
                            {
                            ...
                            if (bOwnsMutex)
                            {
                                ...
                                ipcNotifyEvent = new EventWaitHandle(false, EventResetMode.AutoReset, "Primora_IPCResultData_ReadyEvent");
                            }
                            else
                        ...
                        if (bDoSendMsg)
                        {
                            ...
                            SendMessage(hWndPrimoraForm, PrimoraForms.MainWindow.WM_COPYDATA, IntPtr.Zero, ref cds);
        ...
        private void CreateControlService(ArgumentParser parser)
        {
            ...
            {
                rootHub = new Primora.ControlService(parser);

                Primora.Program.rootHub = rootHub;
                requestClient = new HttpClient();
                requestClient.DefaultRequestHeaders.Add("User-Agent", "Primora");
        ...
        public void CreateIPCClassNameMMF(IntPtr hWnd)
        {
            ...
            try
            {
                ...
                if (GetClassName(hWnd, wndClassNameStr, wndClassNameStr.Capacity) != 0 && wndClassNameStr.Length > 0)
                {
                    ...
                    ipcClassNameMMF = MemoryMappedFile.CreateNew("Primora_IPCClassName.dat", 128);
        ...
        private string ReadIPCClassNameMMF()
        {
            ...
            try
            {
                ...
                mmf = MemoryMappedFile.OpenExisting("Primora_IPCClassName.dat");
        ...
        {
            // Cmdline client process calls this to create the MMF file used in inter-process-communications. The background Primora process 
            ...
            try
            {
                ipcResultDataMMF = MemoryMappedFile.CreateNew("Primora_IPCResultData.dat", 256);
                // The MMF file is alive as long this process holds the file handle open
            }
            catch (Exception)
            {
        ...
        {
            // The background Primora process calls this method to write out the result of "-command QueryProfile.device#" command.
            // The cmdline client process reads the result from the Primora_IPCResultData.dat MMF file and sends the result to console output pipe.
            MemoryMappedFile mmf = null;
            MemoryMappedViewAccessor mma = null;
            EventWaitHandle ipcNotifyEvent = null;
            ...
            try
            {
                ipcNotifyEvent = EventWaitHandle.OpenExisting("Primora_IPCResultData_ReadyEvent");

                byte[] buffer = ASCIIEncoding.ASCII.GetBytes(dataStr);
                mmf = MemoryMappedFile.OpenExisting("Primora_IPCResultData.dat");
        ...
        private void SetUICulture(string culture)
        {
            try
            {
                ...
                //Primora.Properties.Resources.Culture = ci;
        ...
        public void ChangeTheme(Primora.AppThemeChoice themeChoice,
            bool fireChanged = true)
        {
            if (themeChoice == Primora.AppThemeChoice.Default)
            {
                Application.Current.Resources.MergedDictionaries.Clear();

                // Attempt to switch theme based on currently selected Windows apps theme mode
                Primora.AppThemeChoice implicitTheme = Primora.Util.SystemAppsUsingDarkTheme() ?
                    Primora.AppThemeChoice.Dark : Primora.AppThemeChoice.Light;
