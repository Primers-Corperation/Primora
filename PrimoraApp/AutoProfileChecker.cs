    ...
    public class AutoProfileChecker
    {
        ...
        public void Process()
        {
            ...
            if (GetTopWindowName(out topProcessName, out topWindowTitle))
            {
                ...
                for (int i = 0, pathsLen = profileHolder.AutoProfileColl.Count; i < pathsLen; i++)
                {
                    ...
                    if (tempEntity.IsMatch(topProcessName, topWindowTitle))
                    {
                        if (autoProfileDebugLogLevel > 0)
                            Primora.AppLogger.LogToGui($"DEBUG: Auto-Profile. Rule#{i + 1}  Path={tempEntity.path}  Title={tempEntity.title}", false, true);
                ...
                if (matchedProfileEntity != null)
                {
                    ...
                    for (int j = 0; j < ControlService.CURRENT_DS4_CONTROLLER_LIMIT; j++)
                    {
                        ...
                        if (tempname != string.Empty && tempname != "(none)")
                        {
                            ...
                            {
                                if (autoProfileDebugLogLevel > 0)
                                    Primora.AppLogger.LogToGui($"DEBUG: Auto-Profile. LoadProfile Controller {j + 1}={tempname}", false, true);
                                ...
                                DS4Device device = Program.rootHub.PrimoraControllers[j];
                                if (device != null)
                                {
                            ...
                            {
                                if (autoProfileDebugLogLevel > 0)
                                    Primora.AppLogger.LogToGui($"DEBUG: Auto-Profile. LoadProfile Controller {j + 1}={tempname} (already loaded)", false, true);
                            }
                        }
                    }
                    ...
                    if (turnOffDS4WinApp)
                    {
                        ...
                        {
                            if (autoProfileDebugLogLevel > 0)
                                Primora.AppLogger.LogToGui($"DEBUG: Auto-Profile. Turning Primora temporarily off", false, true);
                ...
                {
                    if (turnOffTemp && Primora.Global.AutoProfileRevertDefaultProfile)
                    {
                        turnOffTemp = false;
                        if (!App.rootHub.running)
                        {
                            if (autoProfileDebugLogLevel > 0)
                                Primora.AppLogger.LogToGui($"DEBUG: Auto-Profile. Turning Primora on before reverting to default profile", false, true);
                    ...
                    for (int j = 0; j < ControlService.CURRENT_DS4_CONTROLLER_LIMIT; j++)
                    {
                        ...
                        {
                            if (Primora.Global.AutoProfileRevertDefaultProfile)
                            {
                                if (autoProfileDebugLogLevel > 0)
                                    Primora.AppLogger.LogToGui($"DEBUG: Auto-Profile. Unknown process. Reverting to default profile. Controller {j + 1}={Global.ProfilePath[j]} (default)", false, true);
                            ...
                            {
                                if (autoProfileDebugLogLevel > 0)
                                    Primora.AppLogger.LogToGui($"DEBUG: Auto-Profile. Unknown process. Existing profile left as active. Controller {j + 1}={Global.tempprofilename[j]}", false, true);
                            }
                        }
                    }
                }
        ...
        private bool GetTopWindowName(out string topProcessName, out string topWndTitleName)
        {
            ...
            if (autoProfileDebugLogLevel > 0)
                Primora.AppLogger.LogToGui($"DEBUG: Auto-Profile. PID={lpdwProcessId}  Path={topProcessName} | WND={hWnd}  Title={topWndTitleName}", false, true);
        ...
        private void DisplayProfileChange(int ind, string profile)
        {
            switch (Global.autoProfileSwitchNotifyChoice)
            {
                case AutoProfileDisplayProfileSwitchChoices.Log:
                    {
                        string prolog = string.Format(Primora.Properties.Resources.UsingAutoTempProfile, (ind + 1).ToString(), profile);
                        Primora.AppLogger.LogToGui(prolog, false);
                    }
                ...
                case AutoProfileDisplayProfileSwitchChoices.Notification:
                    {
                        string prolog = string.Format(Primora.Properties.Resources.UsingAutoTempProfile, (ind + 1).ToString(), profile);
                        Primora.AppLogger.LogToTray(prolog);
                    }
