...
namespace Primora
{
    ...
    public class DS4Devices
    {
        ...
        // However, not all those are guaranteed to work with Primora app so support is added case by case when users of Primora app tests non-official DS4 gamepads.
                    ...
                    if (hDevice.IsOpen)
                    {
                        ...
                        if (newdev && validSerial)
                        {
                            ...
                            if (!ds4Device.ExitOutputThread)
                            {
                                ...
                                AppLogger.LogToGui($"{Primora.Properties.Resources.FoundController} {ds4Device.getMacAddress()} ({ds4Device.getConnectionType()}) ({ds4Device.DisplayName}).",false);
                            }
                        }
                    }
                }
        ...
        public static IEnumerable<DS4Device> getPrimoraControllers()
        {
            lock (Devices)
            {
