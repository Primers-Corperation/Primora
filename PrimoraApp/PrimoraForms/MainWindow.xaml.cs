    ...
    public partial class MainWindow : Window
    {
        ...
        public void LateChecks(ArgumentParser parser)
        {
            ...
            tempTask = Task.Delay(100).ContinueWith(_ =>
            {
                ...
                if (checkwhen > 0 && DateTime.Now >= Global.LastChecked + TimeSpan.FromHours(checkwhen))
                {
                    ...
                    {
                        Dispatcher.Invoke(() => MessageBox.Show(Strings.FailedToRetrieveLatestVersion, "Primora Updater"));
                    ...
                    Dispatcher.Invoke(() =>
                    {
                        ...
                        if (!string.IsNullOrEmpty(newUpdaterVersion))
                        {
                            Util.StartProcessHelper(
                                $"https://github.com/PrimersCorporation/DS4Updater/releases/tag/v{newUpdaterVersion}/{mainWinVM.updaterExe}");
                        }
                    });
                }
            }
        ...
        private void Check_Version(bool showstatus = false)
        {
                ...
                if (result == MessageBoxResult.Yes)
                {
                    ...
                    else
                    {
                        Dispatcher.Invoke(() =>
                        {
                            ...
                            if (!string.IsNullOrEmpty(newUpdaterVersion))
                            {
                                Util.StartProcessHelper($"https://github.com/PrimersCorporation/DS4Updater/releases/tag/v{newUpdaterVersion}/{mainWinVM.updaterExe}");
                            }
                        });
                    }
                }
            ...
            else
            {
                ...
                if (showstatus)
                {
                    Dispatcher.Invoke(() => MessageBox.Show(Properties.Resources.UpToDate, "Primora Updater"));
                }
            }
        }
        ...
        private void ShowNotification(object sender, Primora.DebugEventArgs e)
        {
            Dispatcher.BeginInvoke((Action)(() =>
            {
...
Suspend support not enabled.", true);
                ...
                case POWER_RESUME:
                    {
                        ...
                        if (wasrunning)
                        {
                            ...
                            Program.rootHub.LogDebug(Primora.Translations.Strings.WakeupFromSuspend);
        ...
        private void UpdateLastStatusMessage(object sender, Primora.DebugEventArgs e)
        {
            ...
            IntPtr lParam, ref bool handled)
        {
            ...
            switch (msg)
            {
                ...
                case WM_COPYDATA:
                {
                            ...
                            if (strData.Length >= 1)
                            {
                                ...
                                else if (strData[0] == "outputslot" && strData.Length >= 3)
                                {
                                    ...
                                    if (tdevice >= 0 && tdevice < ControlService.MAX_DS4_CONTROLLER_COUNT)
                                    {
                                        ...
                                        PrimoraControl.OutSlotDevice slotDevice = Program.rootHub.OutputslotMan.OutputSlots[tdevice];
                                        if (strData[2] == "unplug")
                                ...
                                else if (strData[0] == "query" && strData.Length >= 3)
                                {
                                    ...
                                    if (tdevice >= 0 && tdevice < ControlService.MAX_DS4_CONTROLLER_COUNT)
                                    {
                                        // Name of the property to query from a profile or Primora app engine
                                        propName = strData[2].ToLower();
                                            ...
                                            else if (propName == "devicevidpid" && App.rootHub.PrimoraControllers[tdevice] != null)
                                                propValue = $"VID={App.rootHub.PrimoraControllers[tdevice].HidDevice.Attributes.VendorHexId}, PID={App.rootHub.PrimoraControllers[tdevice].HidDevice.Attributes.ProductHexId}";
                                            else if (propName == "devicepath" && App.rootHub.PrimoraControllers[tdevice] != null)
                                                propValue = App.rootHub.PrimoraControllers[tdevice].HidDevice.DevicePath;
                                            else if (propName == "macaddress" && App.rootHub.PrimoraControllers[tdevice] != null)
                                                propValue = App.rootHub.PrimoraControllers[tdevice].MacAddress;
                                            else if (propName == "displayname" && App.rootHub.PrimoraControllers[tdevice] != null)
                                                propValue = App.rootHub.PrimoraControllers[tdevice].DisplayName;
                                            else if (propName == "conntype" && App.rootHub.PrimoraControllers[tdevice] != null)
                                                propValue = App.rootHub.PrimoraControllers[tdevice].ConnectionType.ToString();
                                            else if (propName == "exclusivestatus" && App.rootHub.PrimoraControllers[tdevice] != null)
                                                propValue = App.rootHub.PrimoraControllers[tdevice].CurrentExclusiveStatus.ToString();
                                            else if (propName == "battery" && App.rootHub.PrimoraControllers[tdevice] != null)
                                                propValue = App.rootHub.PrimoraControllers[tdevice].Battery.ToString();
                                            else if (propName == "charging" && App.rootHub.PrimoraControllers[tdevice] != null)
                                                propValue = App.rootHub.PrimoraControllers[tdevice].Charging.ToString();
                    ...
                    {
                        // Eat all exceptions in WM_COPYDATA because exceptions here are not fatal for Primora background app
                    }
                    break;
                }
                default: break;
        ...
        private void ImportProfBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            dialog.Filter = "Primora Profile (*.xml)|*.xml";
