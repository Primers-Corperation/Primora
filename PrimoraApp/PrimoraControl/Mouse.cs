...
namespace Primora
{
    public class Mouse : ITouchpadBehaviour
    {
        ...
        protected PrimoraControls pushed = PrimoraControls.None;
        ...
        public virtual void sixaxisMoved(DS4SixAxis sender, SixAxisEventArgs arg)
        {
            ...
            else if (outMode == GyroOutMode.DirectionalSwipe)
            {
                ...
                if (!string.IsNullOrEmpty(ss[0]))
                {
                    ...
                    {
                        s = ss[index];
                        if (andCond && !(int.TryParse(s, out i) && getPrimoraControlsByName(i)))
                        {
                            triggeractivated = false;
                            break;
                        }
                        else if (!andCond && int.TryParse(s, out i) && getPrimoraControlsByName(i))
                        {
        ...
        public bool IsGyroTriggerActive(GyroOutMode mode)
        {
            ...
            if (!string.IsNullOrEmpty(ss[0]))
            {
                ...
                {
                    str = ss[index];
                    if (andCond && !(int.TryParse(str, out i) && getPrimoraControlsByName(i)))
                    {
                        triggeractivated = false;
                        break;
                    }
                    else if (!andCond && int.TryParse(str, out i) && getPrimoraControlsByName(i))
                    {
                        triggeractivated = true;
                        break;
                    }
        ...
        private bool getPrimoraControlsByName(int key)
        {
            switch (key)
            {
                case -1: return true;
        ...
        public virtual void touchesMoved(DS4Touchpad sender, TouchpadEventArgs arg)
        {
            ...
            if (tempMode == TouchpadOutMode.Mouse)
            {
                if (Global.GetTouchActive(deviceNum))
                {
                    ...
                    {
                        if (getPrimoraControlsByName(disArray[i]) == false)
                            tempBool = false;
                    }
        ...
        private void synthesizeMouseButtons()
        {
            ...
            if (Global.GetDS4CSetting(deviceNum, PrimoraControls.TouchLeft).IsDefault &&
                leftDown)
            {
            ...
            if (Global.GetDS4CSetting(deviceNum, PrimoraControls.TouchUpper).IsDefault &&
                upperDown)
            {
                Mapping.MapClick(deviceNum, Mapping.Click.Middle);
            }

            if (Global.GetDS4CSetting(deviceNum, PrimoraControls.TouchRight).IsDefault &&
                rightDown)
            {
                Mapping.MapClick(deviceNum, Mapping.Click.Left);
            }

            if (Global.GetDS4CSetting(deviceNum, PrimoraControls.TouchMulti).IsDefault &&
                multiDown)
            {
